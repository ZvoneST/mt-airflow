...
api_conn = BaseHook.get_connection('agro_meteo_data_conn')
...
'''
insert_query = '''
    INSERT INTO landing.agro_meteo_data (meteo_location_id, agro_meteo_data)
    VALUES (%s, %s)
'''
...
with DAG(
    ...
    agro_meteo_api_active = HttpSensor(
        task_id='agro_meteo_api_active',
        http_conn_id='agro_meteo_data_conn',
        endpoint='api/',
    )
    
    get_store_agro_meteo_data = PythonOperator(
        task_id='get_store_agro_meteo_data',
        python_callable=api_processor.process_data
    )
    
    
    agro_meteo_api_active >> get_store_agro_meteo_data